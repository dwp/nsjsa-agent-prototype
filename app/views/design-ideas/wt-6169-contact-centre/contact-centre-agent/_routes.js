...
const BASE_PATH = 'design-ideas/wt-6169-contact-centre/contact-centre-agent';
const ABS_BASE_PATH = `/${BASE_PATH}`;
const START_PATH = '/design-ideas/wt-6169-contact-centre/contact-centre-agent/nino-search';
...
router.post("/nino-search", function (req, res) {
    ...
    if (answer === "invalid") {
      ...
      res.redirect(`${ABS_BASE_PATH}/view-claim?task=book&claimant=ij&claimStatus=paused&warning`);
 
 
      // Welsh written
    } else if (answer === "welsh1") {
      res.redirect(`${ABS_BASE_PATH}/view-claim?task=book&claimant=ij&claimStatus=awaiting-appointment&warning=welsh1`);

      // Welsh spoken
    } else if (answer === "welsh2") {
      res.redirect(`${ABS_BASE_PATH}/view-claim?task=book&claimant=ij&claimStatus=awaiting-appointment&warning=welsh2`);
  
      // Welsh written and spoken
    } else if (answer === "welsh3") {
      res.redirect(`${ABS_BASE_PATH}/view-claim?task=book&claimant=ij&claimStatus=awaiting-appointment&warning=welsh3`);

      // Cis mismatch
    } else if (answer === "mismatch") {
      res.redirect(`${ABS_BASE_PATH}/view-claim?task=book&claimant=ij&claimStatus=awaiting-appointment&warning=mismatch`);

      // Check cis manually
    } else if (answer === "manually") {
      res.redirect(`${ABS_BASE_PATH}/view-claim?task=book&claimant=ij&claimStatus=awaiting-appointment&warning=manually`);

       // Claimant has appointee
    } else if (answer === "appointee") {
      res.redirect(`${ABS_BASE_PATH}/view-claim?task=book&claimant=ij&claimStatus=awaiting-appointment&warning=appointee`);

      // Member of household potentially violent
    } else if (answer === "violent3") {
      res.redirect(`${ABS_BASE_PATH}/view-claim?task=book&claimant=ij&claimStatus=awaiting-appointment&warning=violent3`);
 
      // Partner pontentially
    } else if (answer === "violent2") {
      res.redirect(`${ABS_BASE_PATH}/view-claim?task=book&claimant=ij&claimStatus=awaiting-appointment&warning=violent2`);
